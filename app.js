...
/* ── Buscador de símbolo TradingView ── */
function searchTVSymbol() {
  ...
  // Si no tiene exchange, intentar detectar automáticamente
  // Ejemplos: AAPL → NASDAQ:AAPL, BTC → BINANCE:BTCUSDT
  const knownMap = {
  ...
  if (!symbol.includes(':') && knownMap[symbol]) {
    symbol = knownMap[symbol];
  } else if (!symbol.includes(':')) {
    // Intentar NASDAQ primero como default
    symbol = 'NASDAQ:' + symbol;
  }

  result.innerHTML = `
    <div style="font-size:11px;color:var(--text2);margin-bottom:8px;">
      Mostrando: <strong style="color:var(--accent)">${symbol}</strong>
      <button onclick="document.getElementById('tv-symbol-result').innerHTML=''" 
        style="background:none;border:none;color:var(--text2);cursor:pointer;font-size:12px;margin-left:8px;">✕ Cerrar</button>
    </div>
    <div class="tradingview-widget-container" id="tv-search-widget">
