...
function toggleGroup(groupId) {
  ...
  // Save state
  try {
    const states = JSON.parse(localStorage.getItem('dygpro_group_states') || '{}');
    states[groupId] = !isCollapsed ? 'collapsed' : 'open';
    localStorage.setItem('dygpro_group_states', JSON.stringify(states));
  } catch(e) {}
}

// Restore group states on load
document.addEventListener('DOMContentLoaded', () => {
  try {
    const states = JSON.parse(localStorage.getItem('dygpro_group_states') || '{}');
    Object.keys(states).forEach(groupId => {
      if (states[groupId] === 'collapsed') {
        const body = document.getElementById(groupId + '-body');
        const chevron = document.getElementById(groupId + '-chevron');
