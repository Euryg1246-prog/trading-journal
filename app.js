...
form.addEventListener("submit", async function(e) {
  ...
  // Botón regresa SIEMPRE sin importar Supabase
...
function importCSV(event) {
  ...
  reader.onload = function(e) {
    ...
    // Guardar local y render inmediato
    try { localStorage.setItem("dygpro_trades_cache", JSON.stringify(trades)); } catch(e) {}
    try { render(); } catch(e) {}
...
function updateAlarmInterval() {
  ...
  if (val > 0) {
    // Pedir permiso de notificaciones solo cuando el usuario activa alarmas
    if (typeof Notification !== 'undefined' && Notification.permission === 'default') {
      Notification.requestPermission().then(perm => {
        if (perm === 'granted') showToast("🔔 Notificaciones activadas", "Recibirás alertas del sistema.");
      });
    }
    ...
    showToast("🔕 Intervalo desactivado", "Las alarmas de intervalo están apagadas.");
  }
}
...
// Notificaciones: se piden solo cuando el usuario activa una alarma
