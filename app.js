...
function loadProfile() {
  ...
  if (currentUser) {
    _supabase.from('profiles').select('trader_name,nickname,capital,broker,instrument,motto').eq('id', currentUser.id).single()
      .then(({ data, error }) => {
...
function openTimePicker(fieldId) {
  ...
  const [curH, curM] = current ? current.split(':').map(Number) : [9, 0];
  ...
  dropdown.innerHTML = `
    <div class="time-picker-preview" id="preview-${fieldId}">
    ...
    <div class="time-picker-cols">
      <div class="time-picker-col">
        ...
        <div style="max-height:200px;overflow-y:auto;display:flex;flex-direction:column;gap:3px;">
          ${hours.map(h => `
            <div class="time-slot ${h === curH ? 'selected' : ''}"
                 id="h-${fieldId}-${h}"
                 onclick="selectHour('${fieldId}', ${h})">
      ...
      <div class="time-picker-col">
        <div class="time-picker-col-label">Min</div>
        <div style="display:flex;flex-direction:column;gap:3px;">
          ${minutes.map(m => `
            <div class="time-slot ${m === Math.round(curM/5)*5 ? 'selected' : ''}"
                 id="m-${fieldId}-${m}"
                 onclick="selectMinute('${fieldId}', ${m})">
              ${String(m).padStart(2,'0')}
            </div>`).join('')}
        </div>
      </div>
    </div>
    <button class="time-picker-confirm" onclick="confirmTimePicker('${fieldId}')">✅ Confirmar</button>
  `;
  ...
  setTimeout(() => {
    const hEl = document.getElementById('h-' + fieldId + '-' + curH);
...
function selectHour(fieldId, h) {
  if (!_pickerState[fieldId]) _pickerState[fieldId] = { h: 9, m: 0 };
  _pickerState[fieldId].h = h;

  // Update UI
  document.querySelectorAll(`[id^="h-${fieldId}-"]`).forEach(el => el.classList.remove('selected'));
  const el = document.getElementById('h-' + fieldId + '-' + h);
  if (el) el.classList.add('selected');

  updatePickerPreview(fieldId);
}

function selectMinute(fieldId, m) {
  if (!_pickerState[fieldId]) _pickerState[fieldId] = { h: 9, m: 0 };
  _pickerState[fieldId].m = m;

  document.querySelectorAll(`[id^="m-${fieldId}-"]`).forEach(el => el.classList.remove('selected'));
  const el = document.getElementById('m-' + fieldId + '-' + m);
  if (el) el.classList.add('selected');

  updatePickerPreview(fieldId);
}

function updatePickerPreview(fieldId) {
  const state = _pickerState[fieldId] || { h: 9, m: 0 };
  const preview = document.getElementById('preview-' + fieldId);
  if (preview) {
    preview.textContent = String(state.h).padStart(2,'0') + ':' + String(state.m).padStart(2,'0');
  }
}

function confirmTimePicker(fieldId) {
  const input = document.getElementById(fieldId);
  ...
  if (fieldId === 'time' || fieldId === 'exitTime') {
    if (typeof calcTradeDuration === 'function') calcTradeDuration();
  }
}

function syncTimePicker(fieldId) {
  const input = document.getElementById(fieldId);
  if (!input || !input.value) return;
  const [h, m] = input.value.split(':').map(Number);
  _pickerState[fieldId] = { h, m };
}
