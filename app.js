...
async function deleteTradesBySource(source) {
  ...
  // Borrar de Supabase
  if (currentUser) {
    await _supabase.from('trades').delete()
      .eq('user_id', currentUser.id)
      .eq('source', source);
  }
  ...
  try { localStorage.setItem('dygpro_trades_cache', JSON.stringify(trades)); } catch(e) {}
  render();
  showToast('🗑️ ' + sourceLabel + ' eliminado', count + ' trades borrados correctamente.');
}
...
function renderDeleteBySourceButtons(containerId) {
  ...
  container.innerHTML = sourcesInTrades.map(src => {
    ...
    return `<button onclick="deleteTradesBySource('${src}')" style="
      ...
      width:auto;
      grid-column:auto;
    " onmouseover="this.style.background='${cfg.color}33'" 
       onmouseout="this.style.background='${cfg.color}18'">
      🗑️ Borrar ${cfg.label}
    </button>`;
  }).join('');
}

// Actualizar botones cada vez que cambian los trades
