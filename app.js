...
function checkOnboarding() {
  const done = localStorage.getItem("dygpro_onboarding_done");
  if (done) return;
  if (!currentUser) return;

  // Solo mostrar si no tiene trades aún
  if (trades.length > 0) {
...
function showOnboarding() {
  ...
  overlay.innerHTML = `
    <div style="
      background: #0a1628; border: 1px solid rgba(56,189,248,0.3);
      border-radius: 24px; padding: 48px; max-width: 560px; width: 100%;
      text-align: center; position: relative;
    ">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:36px;letter-spacing:3px;color:#38bdf8;margin-bottom:8px">
        BIENVENIDO A DYGPRO
      </div>
      <p style="color:#94a3b8;font-size:15px;margin-bottom:40px">
        Tu journal de trading profesional está listo. Elige cómo quieres empezar:
      </p>

      <!-- Steps -->
      <div style="display:flex;flex-direction:column;gap:14px;margin-bottom:40px;text-align:left">

        <div onclick="onboardingAction('import')" style="
          background:#122040;border:1.5px solid rgba(56,189,248,0.25);
          border-radius:14px;padding:20px 24px;cursor:pointer;
          display:flex;align-items:center;gap:16px;
          transition:all .2s;
        " onmouseover="this.style.borderColor='#38bdf8'" onmouseout="this.style.borderColor='rgba(56,189,248,0.25)'">
          <span style="font-size:32px">📥</span>
          <div>
            <div style="font-weight:700;color:#e2e8f0;font-size:15px;margin-bottom:4px">Importar mis trades</div>
            <div style="color:#94a3b8;font-size:13px">Tengo un CSV de Webull, Tradovate, TradingView u otro broker</div>
          </div>
          <span style="margin-left:auto;color:#38bdf8;font-size:20px">→</span>
        </div>

        <div onclick="onboardingAction('manual')" style="
          background:#122040;border:1.5px solid rgba(56,189,248,0.25);
          border-radius:14px;padding:20px 24px;cursor:pointer;
          display:flex;align-items:center;gap:16px;
          transition:all .2s;
        " onmouseover="this.style.borderColor='#38bdf8'" onmouseout="this.style.borderColor='rgba(56,189,248,0.25)'">
          ...
          <div>
            <div style="font-weight:700;color:#e2e8f0;font-size:15px;margin-bottom:4px">Registrar manualmente</div>
            <div style="color:#94a3b8;font-size:13px">Quiero ingresar mis trades uno por uno desde ahora</div>
          </div>
          <span style="margin-left:auto;color:#38bdf8;font-size:20px">→</span>
        </div>

        <div onclick="onboardingAction('explore')" style="
          background:#122040;border:1.5px solid rgba(56,189,248,0.25);
          border-radius:14px;padding:20px 24px;cursor:pointer;
          display:flex;align-items:center;gap:16px;
          transition:all .2s;
        " onmouseover="this.style.borderColor='#38bdf8'" onmouseout="this.style.borderColor='rgba(56,189,248,0.25)'">
          <span style="font-size:32px">👀</span>
          <div>
            <div style="font-weight:700;color:#e2e8f0;font-size:15px;margin-bottom:4px">Explorar primero</div>
            <div style="color:#94a3b8;font-size:13px">Quiero ver cómo funciona el journal antes de agregar datos</div>
          </div>
      ...
      <p style="font-size:12px;color:#475569">
        Puedes configurar tu sistema de trading en <strong style="color:#38bdf8">⚙️ Mi Sistema</strong> cuando quieras
      </p>
    </div>
  `;
...
function onboardingAction(action) {
  ...
  if (action === "import") {
    ...
    if (currentView === "sidebar") {
      showSidebarSection("section-data");
    } else {
      document.getElementById("section-data")?.scrollIntoView({ behavior: "smooth" });
    }
    showToast("📥 Importar trades", "Selecciona tu archivo CSV de Webull, Tradovate, TradingView u otro broker.");
  } else if (action === "manual") {
    // Go to trade form
    if (currentView === "sidebar") {
      showSidebarSection("section-entry");
    } else {
      document.getElementById("section-entry")?.scrollIntoView({ behavior: "smooth" });
    }
    showToast("✏️ Registrar trade", "Llena el formulario con los datos de tu operación.");
  } else {
    showToast("👀 Explorando", "Recorre el journal. Cuando estés listo importa tus trades o regístralos manualmente.");
  }
}
