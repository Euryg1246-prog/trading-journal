...
form.addEventListener("submit", async function(e) {
  ...
  const exitTime = val("exitTime") || "";

  const trade = {
    date, time, exitTime, day, symbol, direction, entry, exit, contracts,
  ...
  // 3. Supabase en segundo plano sin bloquear
  if (currentUser) {
...
function importCSV(event) {
  ...
  reader.onload = function(e) {
    ...
    // Guardar localStorage inmediato
    try { localStorage.setItem("dygpro_trades_cache", JSON.stringify(trades)); } catch(e) {}
    try { render(); } catch(e) { console.log('render error:', e); }

    // Confirmación inmediata
    showToast('✅ ' + formatName + ' importado', result.imported + ' trades · ' + result.skipped + ' ignorados');
...
function calcTradeDuration() {
  const entryTime = document.getElementById('time')?.value;
  const exitTime  = document.getElementById('exitTime')?.value;
  const durEl     = document.getElementById('tradeDuration');
  if (!durEl) return;
  if (!entryTime || !exitTime) { durEl.value = ''; return; }
  ...
  const m = mins % 60;
  durEl.value = h > 0 ? h + 'h ' + m + 'min' : m + ' min';
}

document.addEventListener('DOMContentLoaded', () => {
  const timeEl = document.getElementById('time');
  if (timeEl) timeEl.addEventListener('input', calcTradeDuration);
});
...
document.addEventListener('click', function(e) {
  ...
  if (canvas && !canvas.contains(e.target)) {
    if (equityChart && equityChart.tooltip) {
...
function loadProfile() {
  ...
  if (currentUser) {
    _supabase.from('profiles')
      .select('trader_name,nickname,capital,broker,instrument,motto')
      .eq('id', currentUser.id).single()
      .then(({ data, error }) => {
        if (error || !data) return;
        if (data.trader_name || data.nickname) {
...
function saveProfile() {
  ...
  if (btn) {
    ...
    btn.disabled = true;
    setTimeout(() => { btn.innerHTML = orig; btn.style.background = 'var(--accent)'; btn.disabled = false; }, 2000);
  }
  ...
  if (currentUser) {
    _supabase.from('profiles').upsert({
      id: currentUser.id,
      trader_name: name, nickname,
      capital: parseFloat(capital) || 0,
      broker, instrument, motto
    }, { onConflict: 'id' })
    .then(({ error }) => {
      if (error) showToast('⚠️ Solo guardado local', 'Verifica la base de datos en Supabase.');
      else showToast('✅ Perfil guardado', name || nickname ? 'Bienvenido, ' + (nickname || name) + '!' : 'Perfil actualizado y sincronizado.');
    }).catch(() => showToast('✅ Perfil guardado', 'Datos guardados localmente.'));
  } else {
...
function applyProfileToUI() {
  ...
  const bigInit = document.getElementById('profileAvatarInitials');
  if (bigImg && bigInit) {
    if (photo) { bigImg.src = photo; bigImg.style.display = 'block'; bigInit.style.display = 'none'; }
    else { bigImg.style.display = 'none'; bigInit.style.display = 'block'; bigInit.textContent = initials; }
  }

  const tbImg = document.getElementById('topbar-avatar-img');
  const tbInit = document.getElementById('topbar-avatar-initials');
  const tbName = document.getElementById('topbar-trader-name');
  if (tbImg && tbInit) {
    if (photo) { tbImg.src = photo; tbImg.style.display = 'block'; tbInit.style.display = 'none'; }
    else { tbImg.style.display = 'none'; tbInit.style.display = 'block'; tbInit.textContent = initials; }
  }
  ...
  if (card && (name || nickname || capital || broker)) {
    ...
    const pvInit = document.getElementById('previewInitials');
    if (pvImg && pvInit) {
      if (photo) { pvImg.src = photo; pvImg.style.display = 'block'; pvInit.style.display = 'none'; }
      else { pvImg.style.display = 'none'; pvInit.style.display = 'block'; pvInit.textContent = initials; }
    }
...
function handleProfilePhoto(event) {
  const file = event.target.files[0];
  if (!file) return;
  if (file.size > 2 * 1024 * 1024) { showToast('⚠️ Imagen muy grande', 'Máximo 2MB.'); return; }
  const reader = new FileReader();
  reader.onload = e => {
