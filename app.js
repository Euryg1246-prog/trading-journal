...
form.addEventListener("submit", async function(e) {
  ...
  if (!date) { showToast('⚠️ Campo requerido', 'La fecha es el único campo obligatorio.'); return; }
  ...
  // Botón regresa SIEMPRE — sin importar nada
  ...
  // Supabase en segundo plano — no bloquea nada
  if (currentUser) {
  ...
  reader.onload = function(e) {
    ...
    if (currentUser && newTrades.length > 0) {
      Promise.all(newTrades.map(t => saveTradeToSupabase(t)))
        .then(() => loadTradesFromSupabase())
        .catch(e => console.log('Supabase import error:', e.message));
    }
  };
...
document.getElementById("clearData")?.addEventListener("click", async function() {
  ...
  render();
  showToast('🗑️ Trades eliminados', 'Todos los trades fueron borrados.');
});
...
function initPersonalNotes() {
  ...
  document.getElementById("savePersonalNote")?.addEventListener("click", async function() {
    ...
    if (!date) {
      showToast('⚠️ Falta fecha', 'Selecciona una fecha para la nota.');
      return;
    }
    ...
    renderNotesCalendar();
    showToast('✅ Nota guardada', 'Tu nota personal fue guardada.');
  });
  ...
  document.getElementById("deletePersonalNote")?.addEventListener("click", async function() {
    ...
    if (!personalNotes[date]) {
      showToast('⚠️ Sin nota', 'No hay nota para borrar en esa fecha.');
      return;
    }
...
// También calcular cuando cambia la hora de entrada
document.addEventListener('DOMContentLoaded', () => {
