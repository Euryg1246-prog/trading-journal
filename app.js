...
async function saveTradeToSupabase(trade) {
  if (!currentUser) return;
  const { data, error } = await _supabase.from("trades").insert(tradeToDbRow(trade)).select("id").single();
...
form.addEventListener("submit", async function(e) {
  ...
  try {
    await Promise.race([
      (async () => {
        const newId = await saveTradeToSupabase(trade);
        if (newId) trade._id = newId;
        await Promise.race([
          loadTradesFromSupabase(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('load_timeout')), 6000))
        ]).catch(() => {
          // Si la carga falla, al menos agrega el trade localmente
          trades.push(trade);
          render();
        });
      })(),
