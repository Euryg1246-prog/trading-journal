...
async function fetchLivePrices() {
  ...
  const status = document.getElementById('liveprices-status');
  if (!grid) return;
  ...
  try {
    ...
    if (status) status.textContent = 'Error al cargar precios';
    if (grid) grid.innerHTML = '<div style="color:var(--text2);font-size:13px;padding:10px;">No se pudieron cargar los precios. Intenta de nuevo.</div>';
  }
}
