...
form.addEventListener("submit", async function(e) {
  ...
  // 1. Guardar localmente INMEDIATO — nunca se congela
  trades.push(trade);
  saveTrades();
  render();
  form.reset();

  // 2. Confirmación visual inmediata
  if (submitBtn) {
  ...
  showToast('✅ Trade guardado', `${trade.symbol} ${trade.direction} · ${trade.date} · P/L: $${trade.pl.toFixed(2)}`);

  // 3. Sincronizar con Supabase en segundo plano sin bloquear
  if (currentUser) {
    saveTradeToSupabase(trade).catch(e => console.log('Supabase sync:', e.message));
  }
});
